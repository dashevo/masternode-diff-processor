...
pub fn lookup_masternode_list<'a,
    MNL: Fn(UInt256) -> *const ffi::wrapped_types::MasternodeList + Copy,
    MND: Fn(*const ffi::wrapped_types::MasternodeList) + Copy,
>(
    block_hash: UInt256,
...
>(
    ...
    masternode_list_destroy: MasternodeListDestroy,
    add_insight_lookup: AddInsightBlockingLookup,
    should_process_llmq_of_type: ShouldProcessQuorumTypeCallback,
        ...
        for _i in 0..added_quorums_count {
            if let Some(mut quorum_entry) = QuorumEntry::new(message, *offset) {
                *offset += quorum_entry.length;
                ...
                println!("quorum_creation_error: {:?}", message);
            }
        }
    }
...
fn log_quorums_map(q: HashMap<LLMQType, HashMap<UInt256, QuorumEntry>>, id: String) {
    println!("{} hashes: [", id);
    let mut bmap: BTreeMap<LLMQType, BTreeMap<UInt256, QuorumEntry>> = BTreeMap::new();
    for (qtype, map) in q.clone() {
        let qqtype: u8 = qtype.into();
        let mut bhmap: BTreeMap<UInt256, QuorumEntry> = BTreeMap::new();
...
mod tests {
    ...
    use crate::{ffi, MasternodeList, MerkleTree, mndiff_process, MndiffResult, mnl_diff_process, ToFFI};
    ...
    <'a, BHL: Fn(UInt256) -> u32 + Copy>(files: Vec<String>, chain: ChainType, block_height_lookup: BHL)
                                         -> (bool, HashMap<UInt256, MasternodeList<'a>>) {
        let mut lists: HashMap<UInt256, MasternodeList> = HashMap::new();
        ...
        for file in files {
            ...
            let result = mnl_diff_process(
                ...
                |hash| match lists.get(&hash) {
                    Some(list) => {
                        ...
                        &list_encoded as *const ffi::wrapped_types::MasternodeList
                    },
                    None => null_mut()
                },
                masternode_list_destroy,
            ...
            lists.insert(block_hash.clone(), masternode_list_decoded.clone());
        }
        (true, lists)
    }
    ...
    pub fn assert_diff_result(chain: ChainType, result: MndiffResult) {
        ...
        assert!(result.has_valid_quorum_list_root, "rootQuorumListValid not valid at height {}", bh);
